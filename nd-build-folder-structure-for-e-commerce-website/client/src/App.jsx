...
export default function App() {
  ...
  useEffect(() => {
    api.get('/items').then(({ data }) => setItems(data)).catch(() => setItems([]));
  }, []);
  ...
  return <main className="app-shell">
    <header><span>nd-build-folder-structure-for-e-commerce-website</span><h1>Item Dashboard</h1></header>
    <form className="add-row" onSubmit={addItem}>
      <input placeholder="Add item" value={name} onChange={(event) => setName(event.target.value)} />
      <button>Add</button>
    </form>
